...
public class MSCControleService extends SiconfiService<MatrizSaldoContabeisControle>{
	...
	private static final String API_PATH_MSC_ORCAMENTARIA = "msc_controle";

	private EnteService enteService;

	public void carregarDados(FiltroMSC filtro) {
		
		List<MatrizSaldoContabeisControle> listaMSC = consultarNaApi(filtro);	
	...
	protected void salvarNoBancoDeDados(FiltroMSC filtro, List<MatrizSaldoContabeisControle> listaEntidades) {
	...
	private void excluirMatrizSaldosContabeis(FiltroMSC filtro) {
		...
		StringBuilder queryBuilder = new StringBuilder("DELETE FROM MatrizSaldoContabeisControle msc WHERE msc.exercicio IN (:exercicios) ");
	...
	public List<MatrizSaldoContabeisControle> consultarNaApi(FiltroMSC filtro) {

		List<Integer> listaExercicios = !filtro.isListaExerciciosVazia() ? filtro.getExercicios()
				: EXERCICIOS_DISPONIVEIS;
		List<MatrizSaldoContabeisControle> listaMSC = new ArrayList<>();
	...
	private List<MatrizSaldoContabeisControle> consultarNaApi(FiltroMSC filtro, Integer exercicio) {
	...
	private List<MatrizSaldoContabeisControle> consultarNaApi(FiltroMSC filtro, Integer exercicio, Integer mes) {
	...
	private List<MatrizSaldoContabeisControle> consultarNaApi(FiltroMSC filtro, Integer exercicio, Integer mes,
		...
		List<Integer> listaClassesConta = !filtro.isListaClassesContaVazia()?filtro.getListaClasseConta():CLASSES_CONTAS_CONTROLE;
	...
	public List<MatrizSaldoContabeisControle> consultarNaApi(Integer exercicio, Integer mes, String codigoIbge,
			TipoMatrizSaldoContabeis tipoMatriz, Integer classeConta, TipoValorMatrizSaldoContabeis tipoValorMatriz) {

		List<MatrizSaldoContabeisControle> listaMSC = null;
		try {

			MatrizSaldoContabeisControleResponse mscResponse = obterResponseDaApi(exercicio, mes,
					codigoIbge, tipoMatriz, classeConta, tipoValorMatriz);

			listaMSC = mscResponse != null ? mscResponse.getItems()
					: new ArrayList<MatrizSaldoContabeisControle>();
		} catch (Exception e) {
